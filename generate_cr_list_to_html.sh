...
do
    ...
    then
        ...
        do
			...
			#ssh -p查询的结果一般为2行，如果返回的只有一行，则表明没有查到
            ...
            then
				...
				cr_conponent=`curl -u gaoyx9:gyx050400?? -X GET http://idart.mot.com/rest/api/2/issue/$cr | jq .fields.components[0].name`
				cr_url="https://idart.mot.com/browse/$cr"
				echo "<tr><td><a href="$cr_url">$cr</a></td><td>$description</td><td>$cr_component</td><td>$assign</td></tr>" >> change_list.html
            else
