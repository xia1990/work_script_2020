...
def set_style(name,height,back_color,bold=False):
    style=xlwt.XFStyle()
...
def write_excel():
	...
	for row in range(0,len(row0)):
		sheet1.write(0,row,row0[row],set_style('Times New Roman',400,17,True))
		...
		if content=="":
			if os.path.isdir(project_path):
				...
				if len(cr)==0:
					...
					sheet1.write(commit_index+1,0)
					sheet1.write(commit_index+1,1)
					sheet1.write(commit_index+1,2)
					sheet1.write(commit_index+1,3)
					sheet1.write(commit_index+1,4,project_path)
